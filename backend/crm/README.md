...
#### Детальная карточка клиента (`ClientDetailView`, `crm/clients/detail.html`)

Детальная карточка клиента отображает:

- основные данные клиента: ФИО, телефон, тип (физ/юр), компания (если есть),
  адрес;
- количество заказов клиента;
- общий баланс/долг клиента (`client.total_duty`);
- список заказов клиента (код заказа, оборудование, описание неисправности,
  услуги и статус);
- список покупок (комплектующих) по заказам клиента: дата, магазин, описание,
  стоимость и статус.

Навигация и действия:

- кнопки управления в карточке клиента:
  - «Редактировать» → `/clients/<id>/edit/`
  - «Удалить» → `/clients/<id>/delete/`
  - «К списку» → `/clients/`

![Просмотр клиента](../docs/screenshots/24.png)
---
...
#### Детальная карточка заказа (`OrderDetailView`, `crm/orders/detail.html`)

Детальная карточка заказа отображает:

- код заказа (`order.code`, например `TN-00010`);
- дату создания;
- данные клиента (ФИО, телефон);
- принятое оборудование и описание неисправности;
- список услуг по заказу (через `order.service_lines` с ценой `line.amount`);
- финансовые показатели:
  - `services_total`, `purchases_total`, `total_amount`,
  - `advance`, `paid`, `duty`;
- статус заказа;
- список покупок (комплектующих/ПО), привязанных к заказу

Навигация и действия:

- кнопки управления в карточке заказа:
  - «Редактировать» → `/orders/<id>/edit/`
  - «Удалить» → `/orders/<id>/delete/`
  - «К списку» → `/orders/`

![Просмотр заказа](../docs/screenshots/25.png)
---
### Услуги
...
В таблице покупок отображаются:

- номер заказа, которому принадлежит покупка (`purchase.order.code`) или без заказа ;
- дата создания;
- магазин (`store`);
- описание товара;
- стоимость (`cost`);
...
#### Детальная карточка покупок (`PurchaseDetailView`, `crm/purchases/detail.html`)

Детальная карточка покупки отображает:

- номер заказа, которому принадлежит покупка или без заказа;
- дату создания;
- магазин;
- описание товара;
- стоимость;
- статус покупки;
- список покупок (комплектующих), привязанных к заказу:

Навигация и действия:

- кнопки управления в карточке покупок:
  - «Редактировать» → `/purchases/<id>/edit/`
  - «Удалить» → `/purchases/<id>/delete/`
  - «К списку» → `/purchases/`

![Просмотр покупки](../docs/screenshots/26.png)
